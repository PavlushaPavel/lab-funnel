          ...
          >
            {resultChain.map((stage, i) => (
              <motion.div key={stage} className="contents" variants={reduced ? undefined : listItem}>
                <span className="whitespace-nowrap rounded-md border border-line bg-panel px-3 py-2 t-body-s text-ink">
                  {stage}
                </span>
                {i < resultChain.length - 1 && (
                  <CaretRight weight="regular" size={12} color="var(--ink-faint)" aria-hidden="true" />
                )}
              </motion.div>
            ))}
          </motion.div>
